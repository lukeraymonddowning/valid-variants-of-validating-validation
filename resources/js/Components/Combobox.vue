...
const filteredOptions = computed(() => {
    const options = state.query.value === ''
        ? props.options
        : props.options.filter(value => {
            return value.name.toLowerCase().includes(state.query.toLowerCase())
        });

    return options.length > 0
        ? options
        : [{ id: null, name: state.query }]
});
...
<template>
    <Combobox as="div" :model-value="props.modelValue" @update:modelValue="handleInput($event)" nullable>
        <div class="relative">
            <ComboboxInput
