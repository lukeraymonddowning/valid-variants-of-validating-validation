...
axios.get(route('api.autofill.directors'))
    .then(response => state.directors = response.data)
    .catch(err => state.directors = [])

axios.get(route('api.autofill.movie-ratings'))
    .then(response => {
...
const search = () => debounce(() => Inertia.reload({
    data: {
        search: state.search,
    }
}), 250, 'movie_search');
    ...
    <BreezeAuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <form @submit.prevent="search()" class="flex mb-2">
                ...
                <ul role="list" class="space-y-3">
                    <list-item v-for="movie in props.movies" :key="movie.id" :data="movie"></list-item>
                </ul>
            </div>
        </div>

        <form-modal title="Add a movie"
            ...
            <template #form>
                <form @submit.prevent="submitForm()" id="movie_form" class="space-y-3">
                    <div>
                        <Label for="movie_name">Name</Label>
                        <Input type="text" v-model="form.name" id="movie_name" class="w-full"/>
                        <InputError :message="form.errors.name"/>
                    </div>
                    <div>
                        <Label for="movie_poster">Poster</Label>
                        <ImageUpload v-model="form.poster" id="movie_poster" class="w-full"/>
                        <InputError :message="form.errors.poster"/>
                    </div>
                    <div>
                        <Label for="movie_director_id">Director</Label>
                        <Combobox v-model="director"
                                  :options="state.directors"
                                  id="movie_director_id" class="w-full"/>
                        <InputError :message="form.errors.director_id"/>
                        <InputError :message="form.errors.director_name"/>
                    </div>
                    <div v-if="requiresDirectorFields">
                        <Label for="director_portrait">Director's portrait</Label>
                        <ImageUpload v-model="form.director_portrait" id="director_portrait" class="w-full"/>
                        <InputError :message="form.errors.director_portrait"/>
                    </div>
                    <div v-if="requiresDirectorFields">
                        <Label for="director_date_of_birth">Director's date of birth</Label>
                        <Input type="date" v-model="form.director_born_on" id="director_date_of_birth" class="w-full"/>
                        <InputError :message="form.errors.director_born_on"/>
                        <hr class="mt-4">
                    </div>
                    <div>
                        <Label for="movie_released_on">Released on</Label>
                        <Input type="date" v-model="form.released_on" id="movie_released_on" class="w-full"/>
                        <InputError :message="form.errors.released_on"/>
                    </div>
                    <div class="grid gap-2 md:grid-cols-2">
                        <div>
                            <Label for="movie_rating">Rating</Label>
                            <Select v-model="form.rating" :options="state.ratings" id="movie_rating" class="w-full"/>
                            <InputError :message="form.errors.rating"/>
                        </div>
                        <div>
                            <Label for="movie_runtime">Runtime (in minutes)</Label>
                            <Input type="number" v-model="form.runtime" id="movie_runtime" class="w-full"/>
                            <InputError :message="form.errors.runtime"/>
                        </div>
                    </div>
                    <div>
                        <Label for="movie_plot">Plot</Label>
                        <Textarea v-model="form.plot" id="movie_plot" class="w-full"/>
                        <InputError :message="form.errors.plot"/>
                    </div>
                </form>
            </template>
            <template #actions>
                <Button :disabled="form.processing" type="submit" form="movie_form" class="sm:ml-3">Add Movie</Button>
            </template>
        </form-modal>
    </BreezeAuthenticatedLayout>
</template>
